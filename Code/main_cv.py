...
def main(alg,rank=25,tot_itr=5): #main function
    ...
    K = max(rmd_sz)
    all_datasets = ['dataset'+str(_) for _ in range(1,5)]
    ...
    for dataset in all_datasets:
        ...
        genres = get_genres(M)    #we call it topics instead of genres
        ...
        splt = 0
        for _,te_ind in kf.split(range(R.shape[1])):    #Instead of 5 random split we use 5 fold cross validation
            splt += 1
            ...
            if dataset == 'dataset3' or dataset == 'dataset4':
                users = get_relev_test_users(R_test)
            else:
