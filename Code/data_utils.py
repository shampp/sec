...
dataset_dtls = {
                ...
                'dataset3':{
                    'folder_path'   : '../Data/ml-10m/',
                    'delim' :   '::',
                    'original_ratings_file' :   'ratings.dat',
                    'ratings_file' :   'ratings.csv',
                    'rat_columns' : [0,1,2,3],
                    'original_items_file' : 'movies.dat',
                    'items_file' : 'movies.csv',
                    'itm_columns' : [0,1,2],
                    'tags_file' : 'tags.dat',
                    'tag_columns' : [1,2],  #we are interested in movie id and the corresponding tag
                    },
                'dataset4':{    #in ml-20m there is a default column names, we skip it
                    'folder_path'   : '../Data/ml-20m/',
                    'delim' :   ',',
                    'original_ratings_file' :   'ratings.dat',
                    'ratings_file' :   'ratings.csv',
                    'rat_columns' : [0,1,2,3],
...
###########################################################################################
def get_genres(M):
    genres = list(set(b'|'.join(M['GENRE']).split(b'|')))
    genres = [g.decode("latin1") for g in genres]
    return genres
...
###########################################################################################
def get_relev_test_users(R_test):
    return np.where((R_test>=4).sum(1)!=0)[0]    #list of users who has at least one rating in the test data
###########################################################################################
...
def build_genre_mat(M,genres):
    gen_list = np.core.defchararray.split(M['GENRE'],sep=b'|')
